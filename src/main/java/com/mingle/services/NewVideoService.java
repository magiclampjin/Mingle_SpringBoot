...
public class NewVideoService {
	...
	public List<NewVideoDTO> getLatestVideosFromNetflixKorea() throws IOException {
	    ...
	    for (SearchResult searchResult : searchResultList) {
	        ...
	        newVideo.setUrl("https://www.youtube.com/watch?v=" + videoId);
	        newVideo.setThumbnail(searchResult.getSnippet().getThumbnails().getDefault().getUrl());

	        // 비디오 ID를 사용하여 비디오 객체 가져오기
	        YouTube.Videos.List videoRequest = youtube.videos().list("snippet,statistics");
	        videoRequest.setId(videoId); // 비디오 ID 설정
